...
const validateUserAccount = async (req, res, next) =>{
    const {
        name, lastName, birthDate, CUD, email, password, freelancer, description, address, activate, location, certificates, category
    } = req.body
    ...
    if (!CUD) return res.status(400).json({ error: "Missing CUD" });
    if (!email) return res.status(400).json({ error: "Missing email" });
    if (!password) return res.status(400).json({ error: "Missing password" });
    if (!freelancer && !(typeof "boolean")) return res.status(400).json({ error: "Missing freelancer" });
    if (!location) return res.status(400).json({ error: "Missing location" });
    if (!certificates) return res.status(400).json({ error: "Missing certificates" });
    if(!category) return res.status(404).json({error: "Missing category"})
    
    if (typeof name !== "string") return res.status(400).json({ error: "Name has to be a string" });
    if (typeof lastName !== "string") return res.status(400).json({ error: "LastName has to be a string" });
    if (typeof birthDate !== "string") return res.status(400).json({ error: "BirthDate has to be a string" });
    if (typeof CUD !== "string") return res.status(400).json({ error: "CUD has to be a string" });
    ...
    if (typeof freelancer !== "boolean") return res.status(400).json({ error: "Freelancer has to be a boolean" });
    if (typeof location !== "object") return res.status(400).json({ error: "Location has to be an object" });
    if (typeof certificates !== "object") return res.status(400).json({ error: "Certificates has to be an object" });
    ...
    if(freelancer){
        ...
        if (typeof address !== "string") return res.status(400).json({ error: "Address has to be a string" });
    }
